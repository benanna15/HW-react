...
//bouton modifier
 const handleModify = (tweet) => {
  ...
  const handleCommentClick = (tweetId) => {
    ...
    const updateTotalLikesInHome = (newTotalLikes, isLiked) => {
      setTotalLikes(newTotalLikes);
      setIsLiked(isLiked);
      fetchData();
    };
// maj de likes apres comment
    const updateLikeInHome = () => {
  ...
  const fetchData = async () => {
    try {
      console.log("Avant la requête pour les tweets et les likes");
      ...
      for (const tweet of tweetsData) {
        // Vérifier si le tweet existe dans les likes
        const findTweetValue = likesData.find(
          (like) => like.user_id === id && like.tweet_id === tweet.id
        );
  
        // Faire quelque chose avec le résultat, par exemple, le stocker dans un tableau
        if (findTweetValue) {
          console.log(`Le tweet avec l'ID ${tweet.id} a un like.`);
        } else {
          console.log(`Le tweet avec l'ID ${tweet.id} n'a pas de like.`);
        }
      ...
      console.log("Après le traitement des données");
            ...
            <div key={tweet.id ||  index} className="p-4 mb-4 ">
              ...
              {myTweet && editTweetId === tweet.id ?  (
               ...
               <div className="flex flex-row justify-between mt-4 ml-16">
                   ...
                   <Like id={id} tweet={tweet} likes={likes} redHeart={redHeart} likeCount={tweet.like} findLike={findLike} updateTotalLikesInHome={updateTotalLikesInHome}/> 
<div  onClick={() => handleCommentClick(tweet.id)} >
  ...
  <CommentDialog
    id={id}
    // isOpen={() => (commentClick ? setIsCommentDialogOpen(false) : commentClick)}
   isOpen={commentClick}
