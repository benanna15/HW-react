...
function HomePage() {
  ...
  const handleRegister = () => {
    ...
    const imageFile = stateForm.image || null;

    reader.onloadend = () => {
      const image = reader.result || profil;
      console.log(image);
      axios
      .post("https://promises-cb263f.appdrag.site/api/AddUsers", formData)
      .then(function (response) {
console.log(response.data.Table);      })
      .catch(function (error) {
        ...
        toast.error("Error during registration. Check the console for details.");
      });
  };
