    ...
    image : image
        ...
        .then(function (response) {
         ...
         // setCommentCount((prevCommentCount) => prevCommentCount + 1);
          //setDataComment((prevComments) => [...prevComments, response.data]);
          dispatch(addComment({
            "UserID": id,
            "date" :moment().format(),
            "CommentContent"  : commentText,
...
const handleDelete = (comment) => {
  setCommentToDelete(comment); // Mémorisez le tweet à supprimer.
  setDeleteComment(true); // Affichez la boîte de dialogue de confirmation.
...
const confirmDelete = () => {
  if (commentToDelete) {
    ...
    setDeleteComment(false); // Fermez la boîte de dialogue après la suppression.
  }
};

const cancelDelete = () => {
  setDeleteComment(false); // Annulez la suppression et fermez la boîte de dialogue.
};  
 ...
 useEffect(() => {
  ...
  const updateTotalComment = (tweetId, commentChange) => {
    console.log(tweetId, commentChange);
    axios
